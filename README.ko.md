...
치료 후 진단은 대상 프로젝트의 모듈 캐시를 초기화하여, 오래된 Node.js import가 아닌 실제 변경 코드를 검증합니다. 또한 회귀 감지와 롤백을 결정론적으로 증명하는 안전 치료 검증 랩을 추가했습니다.
...
저장소의 [`examples/safe-repair-lab`](./examples/safe-repair-lab)은 의도적으로 실패하는 진단 하나와 회귀 가드 하나를 포함한 폐기 가능한 검증 프로젝트입니다. 실제 애플리케이션을 건드리지 않고 치료 계획 검토와 승인을 검증할 수 있습니다.

```bash
node bin/vibe-diag.js dashboard --cwd examples/safe-repair-lab --port 7721
```

BYOK를 설정하고 진단을 실행한 뒤 **Plan Repair**를 검토·승인하세요. 결정론적 롤백 검증은 `node examples/safe-repair-lab/verify-rollback.cjs`로 실행할 수 있습니다.
