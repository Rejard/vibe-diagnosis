...
> "`vibe-diagnosis-mcp` 패키지를 글로벌 설치하고, 내 AI 에이전트 설정에 MCP 서버로 등록해 줘."

#### 기존 사용자 (최신 v1.1.1 버전으로 업데이트)
...
> "`vibe-diagnosis-mcp` 패키지를 최신 버전(v1.1.1)으로 강제 업데이트하고 MCP 설정을 다시 불러와(Reload) 줘."
