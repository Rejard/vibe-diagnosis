...
에이전트는 세 가지만 하면 됩니다. `open_dashboard`로 서버를 실행하고, `http://localhost:7700`(이미 사용 중이면 다음 빈 포트)이 실제로 응답하는지 확인한 뒤, `run_diagnostics`를 실행합니다. 이번 작업을 검증할 진단이 없다면 `.diag.js`도 추가해야 합니다. `open_dashboard`가 실패하면 `npx -y vibe-diagnosis dashboard --cwd <프로젝트 경로>`로 서버를 직접 실행합니다.
